...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    ...
    def __init__(self, func, diffusion_func, y0, diffusion_mode, step_size=None, grid_constructor=None, interp="linear", perturb=False, **unused_kwargs):
    ...
    def integrate(self, t):
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            ...
            ################### edit for new sde-net ##################
            if self.diffusion_mode == "fixed":
                diffusion = self.diffusion_func
            elif self.diffusion_mode == "only_x":
                diffusion = self.diffusion_func(y0)
            elif self.diffusion_mode == "x_t":
                diffusion = self.diffusion_func(t, y0)
            
            y1 = y0 + dy + diffusion * torch.sqrt(dt) * torch.randn_like(diffusion)
            ################### edit for new sde-net ##################
